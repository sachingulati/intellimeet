...
jQuery(function ($) {
    ...
    $("#survey_container").wizard({
        ...
        afterForward: function (event, state) {
            ...
            var stepIndex = currentState.stepIndex;
            if (stepIndex == 3) {
                var $confirmOpUl = $('ul.selected');
                $confirmOpUl.html('');
                $('input[type=radio]:checked').each(function () {
                    var selectedOp = $(this).parent('div').siblings('label').find('span').text();
                    $confirmOpUl.append('<li><i class="fa fa-check-square-o fa-2x"></i>&nbsp;' + selectedOp + '</li>');
                });
            }
        }
